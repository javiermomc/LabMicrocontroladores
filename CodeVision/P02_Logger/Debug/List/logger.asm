...
_0x0:
	...
	.DB  0x25,0x69,0x0,0x46,0x65,0x63,0x68,0x61
	.DB  0x3A,0x25,0x30,0x32,0x69,0x2F,0x25,0x30
	.DB  0x32,0x69,0x2F,0x25,0x30,0x32,0x69,0x20
	.DB  0x56,0x31,0x3A,0x25,0x69,0x2E,0x25,0x69
	.DB  0x2C,0x20,0x56,0x32,0x3A,0x25,0x69,0x2E
	.DB  0x25,0x69,0x0
_0x2020003:
	.DB  0x1
_0x20200FE:
...
; 0000 005C void sd(char NombreArchivo[], char *TextoEscritura[]){
_sd:
; .FSTART _sd
; 0000 005D 
; 0000 005E 
...
; 0000 0064         if (res==FR_OK){
	...
	RJMP _0x28
; 0000 0065 
; 0000 0066             f_read(&archivo, buffer, sizeof(buffer)-1,&br); //leer archivo en buffer
	CALL SUBOPT_0x8
	LDI  R30,LOW(_buffer)
	...
	CALL _f_read
; 0000 0067 
; 0000 0068             f_lseek(&archivo,archivo.fsize);
	CALL SUBOPT_0x8
	__GETD2MN _archivo,10
	CALL _f_lseek
; 0000 0069 
; 0000 006A             //buffer[0] = 0x0D;                //Carriage return
; 0000 006B             //buffer[1] = 0x0A;                //Line Feed
; 0000 006C             f_write(&archivo,buffer,2,&br);
	CALL SUBOPT_0x8
	LDI  R30,LOW(_buffer)
	LDI  R31,HIGH(_buffer)
	ST   -Y,R31
	...
	LDI  R27,HIGH(_br)
	CALL _f_write
; 0000 006D             /*Escribiendo el Texto*/
; 0000 006E             sizeTExt = sizeof(text);
	LDI  R30,LOW(32)
	LDI  R31,HIGH(32)
	STS  _sizeTExt,R30
	STS  _sizeTExt+1,R31
; 0000 006F             f_write(&archivo,&TextoEscritura,32,&br1);   // Write of TextoEscritura
	CALL SUBOPT_0x8
	LDD  R30,Y+2
	LDD  R31,Y+2+1
	ST   -Y,R31
...
; 0000 0072         else{
	RJMP _0x29
_0x28:
; 0000 0073             StringLCD("Archivo NO Encontrado");
	__POINTW2FN _0x0,0
	RCALL _StringLCD
; 0000 0074         }
_0x29:
...
; 0000 0076     else{
	RJMP _0x2A
_0x27:
; 0000 0077          StringLCD("Drive NO Detectado");
	__POINTW2FN _0x0,22
	RCALL _StringLCD
; 0000 0078     }
_0x2A:
...
;
;void updateClock(){
; 0000 0081 void updateClock(){
_updateClock:
; .FSTART _updateClock
; 0000 0082     rtc_get_time(&H, &M, &S);
	LDI  R30,LOW(_H)
	LDI  R31,HIGH(_H)
	ST   -Y,R31
	ST   -Y,R30
	...
	LDI  R27,HIGH(_S)
	CALL _rtc_get_time
; 0000 0083     rtc_get_date(&D, &Mes, &A);
	LDI  R30,LOW(_D)
	LDI  R31,HIGH(_D)
	ST   -Y,R31
	ST   -Y,R30
...
;// LCD
;void printTime(){
; 0000 0087 void printTime(){
_printTime:
; .FSTART _printTime
; 0000 0088     sprintf(time, "%02i:%02i:%02i V1: %i.%i", H, M, S, v1I,v1D);
	LDI  R30,LOW(_time)
	LDI  R31,HIGH(_time)
	ST   -Y,R31
	ST   -Y,R30
	...
	CALL SUBOPT_0xB
	CALL SUBOPT_0xC
; 0000 0089     MoveCursor(0,0);
	LDI  R26,LOW(0)
	CALL SUBOPT_0xD
; 0000 008A     StringLCDVar(time);
; 0000 008B     sprintf(time, "%02i:%02i:%02i V2: %i.%i", D, Mes, A, v2I,v2D);
	LDI  R30,LOW(_time)
	LDI  R31,HIGH(_time)
	ST   -Y,R31
	ST   -Y,R30
...
; 0000 0096 // ADC Voltage Reference: Int., cap. on AREF
; 0000 0097 // ADC High Speed Mode: On
; 0000 0098 // Digital input buffers on ADC0: On, ADC1: On, ADC2: On, ADC3: On
; 0000 0099 // ADC4: On, ADC5: On, ADC6: Off, ADC7: Off
; 0000 009A DIDR0=(1<<ADC7D) | (1<<ADC6D) | (0<<ADC5D) | (0<<ADC4D) | (0<<ADC3D) | (0<<ADC2D) | (0<<ADC1D) | (0<<ADC0D);
	LDI  R30,LOW(192)
	STS  126,R30
; 0000 009B ADMUX=ADC_VREF_TYPE;
	STS  124,R30
; 0000 009C ADCSRA=(1<<ADEN) | (0<<ADSC) | (0<<ADATE) | (0<<ADIF) | (0<<ADIE) | (0<<ADPS2) | (0<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(129)
	STS  122,R30
; 0000 009D ADCSRB=(1<<ADHSM);
	LDI  R30,LOW(128)
	STS  123,R30
; 0000 009E 
; 0000 009F // LCD
...
; 0000 00A4 
; 0000 00A5 // SD
; 0000 00A6 // Código para hacer una interrupción periódica cada 10ms
; 0000 00A7 // Timer/Counter 1 initialization
; 0000 00A8 // Clock source: System Clock
; 0000 00A9 // Clock value: 1000.000 kHz
; 0000 00AA // Mode: CTC top=OCR1A
; 0000 00AB // Compare A Match Interrupt: On
; 0000 00AC TCCR1B=0x09;
	LDI  R30,LOW(9)
	STS  129,R30
; 0000 00AD OCR1AH=19999/256;
	LDI  R30,LOW(78)
	STS  137,R30
; 0000 00AE OCR1AL=19999%256;   //20000cuentas a 0.5us por cuenta=10mseg
	LDI  R30,LOW(31)
	STS  136,R30
; 0000 00AF TIMSK1=0x02;
	LDI  R30,LOW(2)
	STS  111,R30
; 0000 00B0 SetupLCD();
	CALL _SetupLCD
; 0000 00B1 #asm("sei")
	sei
; 0000 00B2 /* Inicia el puerto SPI para la SD */
; 0000 00B3 disk_initialize(0);
	LDI  R26,LOW(0)
...
; 0000 00B4 delay_ms(200);
	...
	CALL _delay_ms
; 0000 00B5 
; 0000 00B6 // First actions
; 0000 00B7 PORTC = 0xFF;
	LDI  R30,LOW(255)
	OUT  0x8,R30
; 0000 00B8 
; 0000 00B9 GS = S + STM;
...
_0x2C:
; 0000 00C0     {
; 0000 00C1     // Please write your application code here
; 0000 00C2         // Verify the correct range on clock time
; 0000 00C3 
; 0000 00C4         // ADC
; 0000 00C5         updateADC();
	RCALL _updateADC
; 0000 00C6 
; 0000 00C7         // Clock
; 0000 00C8         updateClock();
	RCALL _updateClock
; 0000 00C9         printTime();
	RCALL _printTime
; 0000 00CA 
; 0000 00CB         // If alarm is on, switch will turn alarm off without
; 0000 00CC         //  changing the default variable
; 0000 00CD         if(!PINC.0){
	SBIC 0x6,0
	RJMP _0x2F
; 0000 00CE             H++;
...
; 0000 00E2             A++;
	...
	SUBI R30,-LOW(1)
	CALL SUBOPT_0x17
; 0000 00E3             rtc_set_date(D, Mes, A);
; 0000 00E4         }
; 0000 00E5         if(!PINC.5){
_0x34:
	SBIC 0x6,5
...
; 0000 00FC             D=0;
	LDI  R30,LOW(0)
	CALL SUBOPT_0x15
; 0000 00FD             rtc_set_date(D, Mes, A);
; 0000 00FE         }
; 0000 00FF         if(Mes>12){
_0x3B:
	LDS  R26,_Mes
...
; 0000 0100             Mes=0;
	LDI  R30,LOW(0)
	CALL SUBOPT_0x16
; 0000 0101             rtc_set_date(D, Mes, A);
; 0000 0102         }
; 0000 0103         if(A>25){
_0x3C:
	LDS  R26,_A
...
; 0000 0104             A=00;
	LDI  R30,LOW(0)
	CALL SUBOPT_0x17
; 0000 0105             rtc_set_date(D, Mes, A);
; 0000 0106         }
; 0000 0107         if(S == GS){
_0x3D:
	LDS  R30,_GS
	...
	BRNE _0x3E
; 0000 0108         //SD here
; 0000 0109             sprintf(text, "Fecha:%02i/%02i/%02i V1:%i.%i, V2:%i.%i", D, Mes, A, v1I, v1D, v2I, v2D);
	LDI  R30,LOW(_text)
	LDI  R31,HIGH(_text)
	ST   -Y,R31
	ST   -Y,R30
