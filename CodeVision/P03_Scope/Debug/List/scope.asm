...
; .FSTART _timer1_compa_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 0053 disk_timerproc();
	CALL _disk_timerproc
; 0000 0054 /* MMC/SD/SD HC card access low level timing function */
; 0000 0055 }
	LD   R30,Y+
	OUT  SREG,R30
	LD   R30,Y+
...
;
;void sd_openDrive(){
; 0000 0064 void sd_openDrive(){
_sd_openDrive:
; .FSTART _sd_openDrive
; 0000 0065     if (f_mount(0,&drive)!=FR_OK){
	LDI  R30,LOW(0)
	ST   -Y,R30
	LDI  R26,LOW(_drive)
	LDI  R27,HIGH(_drive)
...
; 0000 0068         StringLCD("Drive no detectado");
	__POINTD2FN _0x0,0
	RCALL _StringLCD
; 0000 0069     }
; 0000 006A }
...
; .FEND
;void sd_closeDrive(){
; 0000 006B void sd_closeDrive(){
_sd_closeDrive:
; .FSTART _sd_closeDrive
; 0000 006C     f_mount(0, 0); //Cerrar drive de SD
	LDI  R30,LOW(0)
	ST   -Y,R30
	LDI  R26,LOW(0)
	LDI  R27,0
...
; 0000 006D     EraseLCD();
	CALL SUBOPT_0x7
; 0000 006E     MoveCursor(0,0);
; 0000 006F     StringLCD("Drive cerrado");
	__POINTD2FN _0x0,19
	RCALL _StringLCD
...
; 0000 007D     // ADC Clock frequency: 1000.000 kHz
; 0000 007E     // ADC Voltage Reference: Int., cap. on AREF
; 0000 007F     // ADC High Speed Mode: On
; 0000 0080     // Digital input buffers on ADC0: On, ADC1: On, ADC2: On, ADC3: On
; 0000 0081     // ADC4: On, ADC5: On, ADC6: On, ADC7: Off
; 0000 0082     DIDR0=(1<<ADC7D) | (0<<ADC6D) | (0<<ADC5D) | (0<<ADC4D) | (0<<ADC3D) | (0<<ADC2D) | (0<<ADC1D) | (0<<ADC0D);
	LDI  R30,LOW(128)
	STS  126,R30
; 0000 0083     ADMUX=ADC_VREF_TYPE;
	LDI  R30,LOW(192)
	STS  124,R30
; 0000 0084     ADCSRA=(1<<ADEN) | (0<<ADSC) | (0<<ADATE) | (0<<ADIF) | (0<<ADIE) | (1<<ADPS2) | (0<<ADPS1) | (0<<ADPS0);
	LDI  R30,LOW(132)
	STS  122,R30
; 0000 0085     ADCSRB=(1<<ADHSM);
	LDI  R30,LOW(128)
	STS  123,R30
; 0000 0086 
; 0000 0087     // Trigger for scope shot
; 0000 0088     PORTD.7 = 1;
	SBI  0xB,7
; 0000 0089 
; 0000 008A     // SD
; 0000 008B     CLKPR=0x80;
	STS  97,R30
; 0000 008C     CLKPR=0;        //16 MHz
	LDI  R30,LOW(0)
	STS  97,R30
; 0000 008D 
; 0000 008E     // Código para hacer una interrupción periódica cada 10ms
; 0000 008F 
; 0000 0090     TCCR1B=0x0A;             //  CK/8
	LDI  R30,LOW(10)
	STS  129,R30
; 0000 0091     OCR1AH=19999/256;
	LDI  R30,LOW(78)
	STS  137,R30
; 0000 0092     OCR1AL=19999%256;
	LDI  R30,LOW(31)
	STS  136,R30
; 0000 0093     TIMSK1=0x02;
	LDI  R30,LOW(2)
	STS  111,R30
; 0000 0094     StringLCD("...");
	__POINTD2FN _0x0,33
...
; 0000 0099     while (1) {
_0x31:
; 0000 009A     // Please write your application code here
; 0000 009B     StringLCD("Welcome");
	__POINTD2FN _0x0,37
	RCALL _StringLCD
; 0000 009C         if (PIND.7 == 0){
	SBIC 0x9,7
	RJMP _0x34
; 0000 009D             EraseLCD();
	RCALL _EraseLCD
; 0000 009E             StringLCD("Starting     ");
	__POINTD2FN _0x0,45
	RCALL _StringLCD
; 0000 009F             scope();
	RCALL _scope
; 0000 00A0             freq = frequency();
	RCALL _frequency
	__PUTW1R 5,6
; 0000 00A1             sd_openDrive();
	RCALL _sd_openDrive
; 0000 00A2             if(f_open(&archivo,"0:BASE.BMP", FA_OPEN_EXISTING | FA_READ)==FR_OK){
	CALL SUBOPT_0x8
	__POINTW1MN _0x36,0
	ST   -Y,R31
	ST   -Y,R30
	...
	BREQ PC+2
	RJMP _0x35
; 0000 00A3                 f_read(&archivo, encabezado, 64, &br);
	CALL SUBOPT_0x8
	LDI  R30,LOW(_encabezado)
	LDI  R31,HIGH(_encabezado)
	CALL SUBOPT_0x9
	CALL _f_read
; 0000 00A4                 f_close(&archivo);
	LDI  R26,LOW(_archivo)
	LDI  R27,HIGH(_archivo)
	CALL _f_close
; 0000 00A5                 if (f_open(&archivo,fileName, FA_CREATE_ALWAYS | FA_WRITE)==FR_OK){
	CALL SUBOPT_0x8
	...
	BREQ PC+2
	RJMP _0x37
; 0000 00A6                     f_write(&archivo,encabezado,sizeof(encabezado),&br);     //Escribir encabezado
	CALL SUBOPT_0x8
	LDI  R30,LOW(_encabezado)
	LDI  R31,HIGH(_encabezado)
	ST   -Y,R31
...
; 0000 00A9                             output[j/2] = 0xFF;
	...
	LDI  R26,LOW(255)
	STD  Z+0,R26
; 0000 00AA                             if(buffer[j]*256/1024==i){
	MOV  R30,R3
	CALL SUBOPT_0x6
	CALL __GETW1P
	MOV  R31,R30
...
_0x3E:
	...
	AND  R30,R0
	BREQ _0x41
; 0000 00B2                              output[1]=Letra0[18-i];
	CALL SUBOPT_0xB
...
; 0000 00B3                              output[2]=Letra1[18-i];
	CALL SUBOPT_0xB
...
; 0000 00B4                              output[3]=LetraH[18-i];
	CALL SUBOPT_0xB
...
; 0000 00B5                              output[4]=LetraZ[18-i];
	CALL SUBOPT_0xB
...
_0x41:
	...
	RJMP _0x3C
_0x3D:
; 0000 00B8                         f_write(&archivo,output,sizeof(output),&br);
	CALL SUBOPT_0x8
	LDI  R30,LOW(_output)
...
; 0000 00BA                     EraseLCD();
	RCALL _EraseLCD
; 0000 00BB                     StringLCD("Listo      ");
	__POINTD2FN _0x0,70
	RCALL _StringLCD
; 0000 00BC                     f_close(&archivo);
	LDI  R26,LOW(_archivo)
	LDI  R27,HIGH(_archivo)
	CALL _f_close
; 0000 00BD                     delay_ms(2000);
	LDI  R26,LOW(2000)
	LDI  R27,HIGH(2000)
	CALL _delay_ms
; 0000 00BE                 }else{
...
; 0000 00BF                     EraseLCD();
	CALL SUBOPT_0x7
; 0000 00C0                     MoveCursor(0,0);
; 0000 00C1                     StringLCD("File error");
	__POINTD2FN _0x0,82
	RCALL _StringLCD
; 0000 00C2                 }
_0x42:
...
; 0000 00C4                EraseLCD();
	CALL SUBOPT_0x7
; 0000 00C5                MoveCursor(0,0);
; 0000 00C6                StringLCD("base.bmp error");
	__POINTD2FN _0x0,93
	RCALL _StringLCD
; 0000 00C7             }
_0x43:
; 0000 00C8             sd_closeDrive();
