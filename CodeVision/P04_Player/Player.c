...
void GetInfo(unsigned char NombreTexto[]){     
    if (f_open(&archivo, NombreTexto, FA_OPEN_ALWAYS | FA_READ)==FR_OK){
        //muestras = (long)f_size(&archivo);    // f_size does not exist in this revision
        f_lseek(&archivo, muestras-323);
        ...
        if (f_mount(0,&drive)==FR_OK){    
           ...
           i=0; 
            while(1){
                if(i>6)
                    i=0; 
                sprintf(fileName, "0:S%02u.WAV", i);
                ...
                // GetInfo(fileName); // Problems while getting the file
