...
; 0000 0045     Temp = temperature;
	LDI  R26,LOW(_Temp)
	LDI  R27,HIGH(_Temp)
	RCALL __CFD1U
	ST   X+,R30
	ST   X,R31
; 0000 0046     // TEMPORAL print statement for DEV purposes only
; 0000 0047 }
	RET
; .FEND
;
...
; 0000 004E void tono(float freq){
_tono:
; .FSTART _tono
; 0000 004F     if(freq == 0)
	RCALL __PUTPARD2
...
; 0000 0051     else{
	RJMP _0x26
_0x25:
; 0000 0052         float cuentas;
; 0000 0053         unsigned int cuentasEnt;
; 0000 0054 
; 0000 0055         cuentas = 500000.0/freq;
	SBIW R28,6
;	freq -> Y+6
;	cuentas -> Y+2
;	cuentasEnt -> Y+0
	...
	RCALL __DIVF21
	__PUTD1S 2
; 0000 0056         cuentasEnt = cuentas;
	MOVW R26,R28
	RCALL __CFD1U
	ST   X+,R30
	ST   X,R31
; 0000 0057         if(cuentas-cuentasEnt>=0.5)
	LD   R30,Y
	LDD  R31,Y+1
	__GETD2S 2
	CLR  R22
	...
	RCALL __CMPF12
	BRLO _0x27
; 0000 0058             cuentasEnt++;
	LD   R30,Y
	LDD  R31,Y+1
	ADIW R30,1
	ST   Y,R30
	STD  Y+1,R31
; 0000 0059         OCR1AH=(cuentasEnt-1)/256;
_0x27:
	LD   R30,Y
	LDD  R31,Y+1
	SBIW R30,1
	...
	LDI  R31,0
	STS  137,R30
; 0000 005A         OCR1AL=(cuentasEnt-1)%256;
	LD   R30,Y
	SUBI R30,LOW(1)
	STS  136,R30
; 0000 005B         TCCR1A=0x40;    // Timer 1 en modo de CTC
	LDI  R30,LOW(64)
	STS  128,R30
; 0000 005C         TCCR1B=0x09;    // Timer en CK (sin pre-escalador)
	LDI  R30,LOW(9)
	STS  129,R30
; 0000 005D     }
	ADIW R28,6
...
;// LCD
;void printTime(){
; 0000 0071 void printTime(){
_printTime:
; .FSTART _printTime
; 0000 0072     sprintf(time, "H:%02i:%02i:%02i T:%02i", H, M, S, Temp);
	RCALL SUBOPT_0x3
	__POINTW1FN _0x0,0
	ST   -Y,R31
	ST   -Y,R30
	...
	RCALL _sprintf
	ADIW R28,20
; 0000 0073     MoveCursor(0,0);
	LDI  R30,LOW(0)
	ST   -Y,R30
	LDI  R26,LOW(0)
	RCALL SUBOPT_0x5
; 0000 0074     StringLCDVar(time);
; 0000 0075     sprintf(time, "A: %02i:%02i   ", AH, AM);
	RCALL SUBOPT_0x3
	__POINTW1FN _0x0,24
	ST   -Y,R31
	ST   -Y,R30
...
; 0000 0083 // ADC Clock frequency: 1000.000 kHz
; 0000 0084 // ADC Voltage Reference: Int., cap. on AREF
; 0000 0085 // ADC High Speed Mode: On
; 0000 0086 // Digital input buffers on ADC0: On, ADC1: On, ADC2: On, ADC3: On
; 0000 0087 // ADC4: On, ADC5: On, ADC6: On, ADC7: Off
; 0000 0088 DIDR0=(1<<ADC7D) | (0<<ADC6D) | (0<<ADC5D) | (0<<ADC4D) | (0<<ADC3D) | (0<<ADC2D) | (0<<ADC1D) | (0<<ADC0D);
	LDI  R30,LOW(128)
	STS  126,R30
; 0000 0089 ADMUX=ADC_VREF_TYPE;
	LDI  R30,LOW(192)
	STS  124,R30
; 0000 008A ADCSRA=(1<<ADEN) | (0<<ADSC) | (0<<ADATE) | (0<<ADIF) | (0<<ADIE) | (0<<ADPS2) | (1<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(131)
	STS  122,R30
; 0000 008B ADCSRB=(1<<ADHSM);
	LDI  R30,LOW(128)
	STS  123,R30
; 0000 008C 
; 0000 008D // LCD
...
; 0000 0095 DDRB.5=1;
	SBI  0x4,5
; 0000 0096 
; 0000 0097 // First actions
; 0000 0098 PORTC = 0x0F;
	LDI  R30,LOW(15)
	OUT  0x8,R30
; 0000 0099 rtc_get_time(&H, &M, &S);
	RCALL SUBOPT_0x6
; 0000 009A printTime();
	RCALL _printTime
; 0000 009B 
; 0000 009C while (1){
_0x2F:
; 0000 009D     // Please write your application code here
; 0000 009E 
; 0000 009F         // Counter
; 0000 00A0         i++;
	LDS  R30,_i
...
; 0000 00A3         updateADC();
	RCALL _updateADC
; 0000 00A4 
; 0000 00A5         // Update clock
; 0000 00A6         updateClock();
	RCALL _updateClock
; 0000 00A7         // Print values un LCD display
; 0000 00A8         printTime();
	RCALL _printTime
; 0000 00A9 
; 0000 00AA         // 100 ms delay
; 0000 00AB         if(i%2==0){
	LDS  R26,_i
	CLR  R27
	LDI  R30,LOW(2)
...
_0x34:
; 0000 00B2 
; 0000 00B3         // 500 ms delay, reset counter
; 0000 00B4         if(i%10==0)
_0x32:
	LDS  R26,_i
	CLR  R27
...
; 0000 00B5             i=0;
	LDI  R30,LOW(0)
	STS  _i,R30
; 0000 00B6         // Turns alarm flag on when H, M and S match
; 0000 00B7         if(S==0 && M==AM && H==AH)
_0x35:
	LDS  R26,_S
	CPI  R26,LOW(0x0)
	BRNE _0x37
...
_0x37:
	RJMP _0x36
_0x38:
; 0000 00B8             alarmFlag=1;
	LDI  R30,LOW(1)
	STS  _alarmFlag,R30
; 0000 00B9 
; 0000 00BA         // Clock
; 0000 00BB 
; 0000 00BC         // If alarm is on, switch will turn alarm off without
; 0000 00BD         //  changing the default variable
; 0000 00BE         if(!PINC.0){
_0x36:
	SBIC 0x6,0
	RJMP _0x39
; 0000 00BF             if(alarmFlag==1)
	LDS  R26,_alarmFlag
	CPI  R26,LOW(0x1)
	BRNE _0x3A
; 0000 00C0                 alarmFlag = 0;
	LDI  R30,LOW(0)
	STS  _alarmFlag,R30
; 0000 00C1             else{
	RJMP _0x3B
...
; 0000 00C2                 H++;
	...
	SUBI R30,-LOW(1)
	RCALL SUBOPT_0x7
; 0000 00C3                 rtc_set_time(H, M, S);
; 0000 00C4             }
_0x3B:
; 0000 00C5         }
; 0000 00C6         // If alarm is on, switch will turn alarm off without
; 0000 00C7         //  changing the default variable
; 0000 00C8         if(!PINC.1){
_0x39:
	SBIC 0x6,1
	RJMP _0x3C
; 0000 00C9             if(alarmFlag==1)
	LDS  R26,_alarmFlag
	CPI  R26,LOW(0x1)
	BRNE _0x3D
; 0000 00CA                 alarmFlag = 0;
	LDI  R30,LOW(0)
	STS  _alarmFlag,R30
; 0000 00CB             else{
	RJMP _0x3E
...
; 0000 00CC                 M++;
	...
	SUBI R30,-LOW(1)
	RCALL SUBOPT_0x8
; 0000 00CD                 rtc_set_time(H, M, S);
; 0000 00CE             }
_0x3E:
; 0000 00CF         }
; 0000 00D0         // Verify the correct range on clock time
; 0000 00D1         if(S>59){
_0x3C:
	LDS  R26,_S
	CPI  R26,LOW(0x3C)
...
; 0000 00DC             H=0;
	LDI  R30,LOW(0)
	RCALL SUBOPT_0x7
; 0000 00DD             rtc_set_time(H, M, S);
; 0000 00DE             updateClock();
	RCALL _updateClock
; 0000 00DF         }
; 0000 00E0         if(AM>59){
_0x41:
...
; 0000 00F0         }
_0x46:
; 0000 00F1         // If alarm is on, switch will turn alarm off without
; 0000 00F2         //  changing the default variable
; 0000 00F3         if(!PINC.3){
_0x44:
	SBIC 0x6,3
	RJMP _0x47
; 0000 00F4             if(alarmFlag==1)
	LDS  R26,_alarmFlag
	CPI  R26,LOW(0x1)
	BRNE _0x48
; 0000 00F5                 alarmFlag = 0;
	LDI  R30,LOW(0)
	STS  _alarmFlag,R30
; 0000 00F6             else
	RJMP _0x49
...
; 0000 00F8         }
_0x49:
; 0000 00F9         // Verify the correct range on alarm time
; 0000 00FA         if(AM>59)
_0x47:
	RCALL SUBOPT_0x9
	BRLO _0x4A
